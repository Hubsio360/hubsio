...
export function BusinessProcessStep({
  ...
  return <>
      <DialogHeader>
        ...
        <DialogDescription>Identifiez les processus métier clés à inclure dans l'analyse de risques. Un processus métier est une activité clé de l'organisation lui permettant de délivrer son service dans les conditions nominales.</DialogDescription>
      </DialogHeader>
      
      {generatingScenarios ? <div className="py-8 space-y-6">
          ...
          <div className="flex gap-2">
            <Input value={newProcess} onChange={e => setNewProcess(e.target.value)} placeholder="Ajouter un processus métier" className="flex-1" onKeyDown={e => {
          if (e.key === 'Enter') {
            e.preventDefault();
            handleAddProcess();
          }
        }} />
            <Button onClick={handleAddProcess} disabled={!newProcess.trim()} size="sm">
          ...
          <div className="rounded-md border p-4">
            ...
            {businessProcesses.length === 0 ? <p className="text-sm text-muted-foreground italic">
                Aucun processus métier ajouté. Veuillez en ajouter au moins un.
              </p> : <ScrollArea className="h-[200px] pr-4">
                <div className="space-y-2">
                  {businessProcesses.map(process => <div key={process.id} className="flex items-center justify-between rounded-md border p-2">
                      <span className="text-sm">{process.name}</span>
                      <Button variant="ghost" size="sm" onClick={() => onRemoveProcess(process.id)} className="h-7 w-7 p-0">
                        <X className="h-4 w-4" />
                      </Button>
                    </div>)}
                </div>
      ...
      <DialogFooter>
        ...
        <Button onClick={onNext} disabled={businessProcesses.length === 0 || generatingScenarios}>
          {generatingScenarios ? <>
